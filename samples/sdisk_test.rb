...
def sep(b, outer = false)
	d = EZ_TYPES::Disk.new(1, 2, Interval[4, 5])
	s_disk(b, d, outer)
end

initial_box = IntervalVector[Interval[-7, 7], Interval[-6, 8]]

[false, true].each do |outer|
	bin, bon, ball = sivia(initial_box, 0.1) {|b| sep(b, outer)}
	...
	f.write "ring_separator_#{outer ? 'out' : 'in'}.png"
end
